      ...
      <radio-group class="radio-group" @change="radioChange">
        ...
        >
          <label class="hj-device-summary_monitor__point-data__item">
            <radio :value="measureData.id" :checked="measureData.checked" color="#886aea"/>
            <span
            ...
            >
              <span
                class="hj-device-summary_monitor__point-data__item__subtitle__value"
              >{{measureData.value}}</span>
              <span
                class="hj-device-summary_monitor__point-data__item__subtitle__unit"
              >{{measureData.unit}}</span>
            </span>
          </label>
          <wux-white-space></wux-white-space>
          <div v-if="measureData.children && measureData.children.length" >
          <hjDeviceSummaryMeasureItemPanel
            v-for="dataItem in measureData.children"
            :key="dataItem.dataId"
            :dataItem="dataItem"
          ></hjDeviceSummaryMeasureItemPanel>
          </div>
...
import hjDeviceSummaryMeasureItemPanel from "./hj-device-summary-measure-item-panel.vue";
  ...
  methods: {
    ...
    proccessChartData(data) {
      ...
      var seriesDataArr = filterLogsList.map(filterLogs => {
        return this.getSeriesTimeDataArr({
          startTime: this.startTime,
          endTime: this.endTime,
    ...
    getMeasureNameAndUnit() {
      if (!this.measureDataList.length) {
        ...
        var measureName = checkedObj.measureName;
        var unit = checkedObj.unit;
        if (
          !unit &&
          checkedObj.children &&
          Array.isArray(checkedObj.children) &&
          checkedObj.children.length
        ) {
          unit = checkedObj.children[0].unit;
        }
        return { measureName, unit };
      }
    },
    setChartData(data) {
