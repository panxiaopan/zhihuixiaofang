...
import hjDeviceAssignmentPanel from "./hj-device-assignment-panel.vue";
...
export default {
  ...
  computed: {
    isAuth() {
      ...
      // );
      // return this.$store.state.userPermissionsMap["mp_task_group"];
      return true;
    }
  },
  watch: {
  ...
  methods: {
    ...
    clickPanel(item) {
      ...
      //-- 可注释掉， 只保留  assignmentDetail = {workOrderId:item.workOrderId}
      ...
      //--
      console.log("assignmentDetail", assignmentDetail); // {workOrderId}
    ...
    transformDataToPanelViewData(data, contentlist) {
      ...
      var title = workOrderVO.number;
      var postTitle = (workOrderVO.type && workOrderVO.type.desc) || "";
      var subtitle =
      ...
      if (contentlist) {
        ...
        contentlist = [
          // { label: "工单编号", value: workOrderVO.number },
          { label: "名称", value: workOrderVO.title },
          {
            label: "描述",
            value: workOrderVO.description
          },
          {
            label: "状态",
            value: workOrderVO.state.desc
          }
        ];
      }

      var foot =
        (workOrderVO.executors &&
          workOrderVO.executors[0] &&
          workOrderVO.executors[0].name) ||
        "";
    ...
    initData({ cb, ...params }) {
      ...
      newParams.id = newParams.id || this.id;
      newParams.deviceId = newParams.deviceId || this.deviceId;
      ...
      this.$http("socialUnitsDeviceWorkOrders", newParams, {
        method: "get",
