<template>
  <div class="hj-trouble-component">
    <div
      v-if="!isAuth"
    ...
    <wux-filterbar
      v-if="isAuth"
      class="hj-trouble-component__filter"
      :items="filterList"
    ...
    <wux-refresher
      v-if="isAuth"
      id="wux-refresher-hj-trouble-component"
      @pulling="onPulling"
      @refresh="onRefresh"
    >
      <hjTroublePanel
...
// import hjFilter from "@/components/hj-trouble-component/hj-filter.vue";
...
export default {
  ...
  data() {
    return {
      ...
      // selectContainerClass: 'hj-trouble-component__select__container__wrapper__shadow',
  ...
  methods: {
    ...
    stopWuxRefresher() {
      $stopWuxRefresher(
        "#wux-refresher-hj-trouble-component",
        this.$root.$mp.page
      );
    },
    // confirm(params) {
    ...
    initData(params = {}, cb) {
      ...
      this.$http(this.queryUrl, params, { method: "get" })
        .then(res => {
          if (cb) {
            cb(res);
          } else {
